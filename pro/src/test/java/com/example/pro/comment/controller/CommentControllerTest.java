...
class CommentControllerTest extends ControllerTest {
    ...
    void deleteComment() throws Exception {
        ...
        ResultActions perform = mockMvc.perform(delete("/comments/{commentId}", 1)
                .contentType(MediaType.APPLICATION_JSON)
                .locale(Locale.KOREAN)
                .characterEncoding(StandardCharsets.UTF_8)
        );
        ...
        perform.andDo(print())
                ...
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.response").value("댓글이 성공적으로 삭제되었습니다."));

        perform.andDo(document("comment delete-success",
                preprocessRequest(prettyPrint()),
                preprocessResponse(prettyPrint()),
                resource(ResourceSnippetParameters.builder()
                        .tag("API-Comment")
    ...
    void deleteCommentNotFound() throws Exception {
        when(authService.loadUser()).thenReturn(member);
        doThrow(new CommentException(CommentErrorCode.COMMENT_NOT_FOUND))
        ...
        perform.andDo(document("comment delete-comment not found",
                preprocessRequest(prettyPrint()),
                preprocessResponse(prettyPrint()),
                resource(ResourceSnippetParameters.builder()
                        .tag("API-Comment")
    ...
    void deleteCommentNotPermitted() throws Exception {
        when(authService.loadUser()).thenReturn(member);
        doThrow(new CommentException(CommentErrorCode.COMMENT_ACCESS_NOT_PERMITTED))
                .when(commentService).deleteComment(any(), any());
        ...
        perform.andDo(document("comment delete-not permitted",
                preprocessRequest(prettyPrint()),
                preprocessResponse(prettyPrint()),
                resource(ResourceSnippetParameters.builder()
                        .tag("API-Comment")
